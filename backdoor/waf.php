...
function filter_dangerous_words($str){
    ...
    $str = str_replace("<", "《", $str);
    $str = str_replace(">", "》", $str);
	$str = str_replace("php", "p", $str);
	$str = str_replace("ini", "iii", $str);
	$str = str_replace("htacc", "iii", $str);
	$str = str_replace("flag", "ffff", $str);
    return $str;
}
...
function write_attack_log($alert){
    ...
    fclose($ffff);
    if($alert == 'GETFLAG'){
        echo 'flag{'.md5(rand()).'}';; //如果请求带有flag关键字，显示假的flag。（2333333）
    }else{
        sleep(3); //拦截前延时3秒
    }
    exit(0);
